...
★ gui/sim_worker.py가 이 콜백 안에서 직접 캡처+emit+페이싱 sleep을 함 —
  Stop/Reset 반응, Status 패널 FPS 갱신이 배치 처리 중엔 멎는 대신 화면이
  실시간으로 갱신됨(트레이드오프).
★ 캡처 하나가 페이싱 예산(gui/sim_worker.py MOTION_REPLAY_FPS 기준)에 근접한
  상태(320x240에서 ~15ms, 변동폭 ±10~15ms)라, 이 값을 낮춰 캡처를 더 자주
  하면 "느린 캡처 한 번 걸릴" 확률도 그만큼 늘어나 오히려 더 자주 끊겨 보일
  수 있음 — 바꾸기 전에 실제로 켜서 체감 확인할 것. 프레임 밀도보다 캡처
  자체의 비용/변동폭을 줄이는 쪽(해상도, 바닥판 크기 등)이 우선순위 높음."""
