...
class Scene:
    ...
    def build(self) -> None:
        ...
        # 쿼드라 TinyRenderer(소프트웨어 래스터라이저)에서 화면을 거의 다 채우며
        # 그려져서 렌더 비용의 대부분(320x240에서 실측 27.7ms 중 ~27ms)을 차지함.
        # 작업영역(WORKSPACE_X/Y, 최대 0.65m)과 수거함(BIN_POSITIONS, 최대 0.6m)을
        # 넉넉히 덮는 4x4m 박스로 대체 — halfExtents z를 아주 얇게 잡고 중심을
        # -half만큼 내려서 윗면이 정확히 z=0에 오게 함(spawner.py가 물체를
        # z=OBJECT_HALF_EXTENT에 스폰하는 전제와 동일).
