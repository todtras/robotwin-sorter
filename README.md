...
> 알파벳순으로 강제 정렬해서 내보내므로, 라벨링할 땐 pet/can/general 순서로
> 작업했더라도 실제 학습 데이터의 클래스 ID는 이 순서로 고정됩니다.

| ID | 클래스 | 대상 | 수거함 | 좌표 |
|---|---|---|---|---|
| 0 | `can` | 알루미늄 캔, 통조림캔 | `green_bin` | (0.00, 0.60, 0) |
| 1 | `general` | 종이컵, 비닐, 티슈, 일회용 수저 | `gray_bin` | (-0.45, 0.45, 0) |
| 2 | `pet` | 투명 페트병, 플라스틱 음료병 | `blue_bin` | (0.45, 0.45, 0) |
